...
def parse_args():
    parser = argparse.ArgumentParser(description='CAD形状连接器 - 创建形状间的通道')
    parser.add_argument('--use-gpu', action='store_true', help='使用GPU加速计算最近点')
    parser.add_argument('--channel-width', type=float, default=10.0, help='连接通道的宽度，默认为5.0')
...
def main():
    ...
    logging.info("======= 开始执行 =======")
    logging.info("该程序已更新：在形状之间的最近点处创建宽度为10的通道")
    # 记录开始时间
    ...
    if cad_solution.create_channels_between_polylines(channel_width=args.channel_width):
        ...
        logging.info(f"连接通道创建成功，耗时: {connect_time:.2f}秒")
    else:
        logging.error("连接通道创建失败")
