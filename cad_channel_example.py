...
from channel_line.cad_channel_builder import CADChannelBuilder

def parse_args():
    parser = argparse.ArgumentParser(description='CAD嵌套实体通道构建器')
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('logs/cad_channel_builder.log', mode='w'),  # 文件输出
            logging.StreamHandler()  # 控制台输出
        ]
    )

    logging.info("======= 开始执行AutoCAD通道构建 =======")
    # 记录开始时间
    ...
    # 创建通道构建器实例
    channel_builder = CADChannelBuilder(use_gpu=args.use_gpu)
    ...
    logging.info("======= 执行完成 =======")
    logging.info(f"日志文件保存在: {os.path.abspath('logs/cad_channel_builder.log')}")
